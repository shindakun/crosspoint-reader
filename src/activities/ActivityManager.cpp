...
void ActivityManager::goToGame() { replaceActivity(std::make_unique<GameTitleActivity>(renderer, mappedInput)); }

void ActivityManager::startGame() {
  if (!GAME_STATE.hasSaveFile()) {
    GAME_STATE.newGame(static_cast<uint32_t>(millis()) ^ 0xDEADBEEF);
  }
  replaceActivity(std::make_unique<GameActivity>(renderer, mappedInput));
}
